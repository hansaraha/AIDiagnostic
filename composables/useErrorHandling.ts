...
export type ErrorType = 'connection' | 'server' | 'validation' | 'notFound' | 'unknown';
...
export default function useErrorHandling() {
  ...
  const handleFetchError = (
    ...
    let message = customMessage || 'Ha ocurrido un error inesperado. Por favor, inténtalo de nuevo.';
    let type: ErrorType = 'unknown';
    
    // Determine error type based on the error object
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      type = 'connection';
      message = 'No hay conexión a internet. Por favor, verifica tu conexión e inténtalo de nuevo.';
    } else if (error instanceof TypeError && error.message.includes('NetworkError')) {
      type = 'connection';
      message = 'Error de conexión. No se puede conectar con el servidor.';
    } else if (error.status === 404 || (error instanceof Response && error.status === 404)) {
      type = 'notFound';
      message = 'El recurso solicitado no está disponible.';
    } else if (error.status >= 500 || (error instanceof Response && error.status >= 500)) {
      type = 'server';
      message = 'Error del servidor. Por favor, inténtalo más tarde.';
    } else if (error.status >= 400 || (error instanceof Response && error.status >= 400)) {
      type = 'validation';
      message = 'Los datos enviados no son válidos. Por favor, revisa la información ingresada.';
    }
  ...
  const withErrorHandling = async <T>(
    ...
    try {
      ...
      const retryFn = async () => {
        const result = await withErrorHandling(fn, errorMessage);
        return result;
      };
  ...
  return {
    error, // Export directly (no readonly) to allow direct modification
    clearError,
    setError,
    handleFetchError,
    withErrorHandling,
