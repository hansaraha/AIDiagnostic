<template>
  <QuestionCard title="¿Has notado mejoras en tu trabajo con IA?" buttonText="CONTINUAR" :disabled="!model"
    @next="$emit('next')">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
      <div v-for="option in aiImprovementsOptions" :key="option.value"
...
import type { AIImprovements } from "~/types/questionnaire";
import QuestionCard from "../ui/QuestionCard.vue";

const props = defineProps<{
  modelValue: AIImprovements | undefined;
}>();
const emit = defineEmits(["update:modelValue", "next"]);

const model = ref<AIImprovements | undefined>(props.modelValue);
...
const { aiImprovementsOptions } = useOptionsData();

function selectOption(value: AIImprovements) {
  model.value = value;
}
</script>
