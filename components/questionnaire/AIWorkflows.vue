<template>
  <QuestionCard title="¿Has desarrollado flujos de trabajo con IA?" buttonText="CONTINUAR" :disabled="!model"
    @next="$emit('next')">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
      <div v-for="option in aiWorkflowsOptions" :key="option.value"
...
import type { AIWorkflows } from "~/types/questionnaire";
import QuestionCard from "../ui/QuestionCard.vue";

const props = defineProps<{
  modelValue: AIWorkflows | undefined;
}>();
const emit = defineEmits(["update:modelValue", "next"]);

const model = ref<AIWorkflows | undefined>(props.modelValue);
...
const { aiWorkflowsOptions } = useOptionsData();

function selectOption(value: AIWorkflows) {
  model.value = value;
}
</script>
