...
import type { AIKnowledgeLevel } from "~/types/questionnaire";
import QuestionCard from "~/components/ui/QuestionCard.vue";

const props = defineProps<{ modelValue?: AIKnowledgeLevel }>();
const emit = defineEmits(["update:modelValue", "next"]);

const { aiKnowledgeLevelOptions: options } = useOptionsData();
const selected = ref<AIKnowledgeLevel | undefined>(props.modelValue);

watch(selected, (val) => emit("update:modelValue", val));

function selectOption(value: AIKnowledgeLevel) {
