<template>
    <QuestionCard title="¿Cuánto tiempo te ahorra la IA semanalmente?" buttonText="CONTINUAR" :disabled="!model"
        @next="$emit('next')">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
            <div v-for="option in aiSavingTimeOptions" :key="option.value"
...
import type { AISavingTime } from "~/types/questionnaire";
import QuestionCard from "../ui/QuestionCard.vue";

const props = defineProps<{
    modelValue: AISavingTime | undefined;
}>();
const emit = defineEmits(["update:modelValue", "next"]);

const model = ref<AISavingTime | undefined>(props.modelValue);
...
const { aiSavingTimeOptions } = useOptionsData();

function selectOption(value: AISavingTime) {
    model.value = value;
}
</script>
