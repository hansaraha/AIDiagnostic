<template>
  <QuestionCard title="¿Tu empresa fomenta el uso de IA?" buttonText="SIGUIENTE" :disabled="!selectedValue"
    @next="$emit('next')">
...
const options = [
  { value: 'active', label: 'Sí, fomenta activamente', emoji: '🚀' },
  { value: 'some_teams', label: 'Solo en algunos equipos', emoji: '👥' },
  { value: 'not_prohibited', label: 'No, pero no lo prohíbe', emoji: '🤷‍♂️' },
  { value: 'prohibited', label: 'No, está prohibido', emoji: '⛔' },
  { value: 'unknown', label: 'No lo sé', emoji: '❓' }
];
