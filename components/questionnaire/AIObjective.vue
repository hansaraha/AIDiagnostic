<template>
  <QuestionCard title="¿Cuál es tu principal objetivo con la IA?" buttonText="CONTINUAR" :disabled="!model"
    @next="$emit('next')">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
      <div v-for="option in aiObjectiveOptions" :key="option.value"
...
import type { AIObjective } from "~/types/questionnaire";
import QuestionCard from "../ui/QuestionCard.vue";

const props = defineProps<{
  modelValue: AIObjective | undefined;
}>();
const emit = defineEmits(["update:modelValue", "next"]);

const model = ref<AIObjective | undefined>(props.modelValue);
...
const { aiObjectiveOptions } = useOptionsData();

function selectOption(value: AIObjective) {
  model.value = value;
}
</script>
