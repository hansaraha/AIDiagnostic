    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
      <div v-for="option in options" :key="option.value" class="border rounded-lg p-3 cursor-pointer border-[#5D49F6]"
        ...
        <div class="flex items-center space-x-3 p-3 sm:p-4 w-full">
          ...
          <div v-if="selected === option.value" class="flex-shrink-0 text-indigo-600 dark:text-indigo-400">
...
import type { MarketingPromptChoice } from "~/types/questionnaire";
import QuestionCard from "~/components/ui/QuestionCard.vue";

const props = defineProps<{ modelValue?: MarketingPromptChoice }>();
const emit = defineEmits(["update:modelValue", "next"]);

const { marketingPromptChoiceOptions: options } = useOptionsData();
const selected = ref<MarketingPromptChoice | undefined>(props.modelValue);

watch(selected, (val) => emit("update:modelValue", val));

function selectOption(value: MarketingPromptChoice) {
  selected.value = value;
}

function emitNext() {
  if (selected.value) emit("next");
}
</script>